...
class RegisterSMA extends BaseController
{
    public function index()
    {
		...
		$data['sekolah2'] = $sekolah->where('tingkat', 'SMA')->orderBy('nama_sekolah', 'ASC')->findAll();

        return view('register_sma', $data);
    }
    ...
    {   
        if (!$this->validate([
            ...
            return redirect()->to('register/sma')->withInput();
        }
        ...
        $user = new UserModel();
        $peserta_sd = new PesertaSMAModel();
        $kode_peserta = $peserta_sd->countAllResults();
        $kode_peserta = sprintf("G-%03d", $kode_peserta);
        $kode_1 = $kode_peserta;
        ...
        //return redirect()->to('register/sma');
    }
}