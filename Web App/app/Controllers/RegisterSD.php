...
class RegisterSD extends BaseController
{
    public function index()
    {
		...
		$data['sekolah2'] = $sekolah->where('tingkat', 'SD')->orderBy('nama_sekolah', 'ASC')->findAll();

        return view('register_sd', $data);
    }
    ...
    {   
        if (!$this->validate([
            ...
            return redirect()->to('register/sd')->withInput();
        }
        ...
        $user = new UserModel();
        $peserta_sd = new PesertaSDModel();
        $kode_peserta = $peserta_sd->countAllResults();
        $kode_peserta = sprintf("E-%03d", $kode_peserta);
        $kode_1 = $kode_peserta;
        ...
        // return redirect()->to('register/sd');
    }
}